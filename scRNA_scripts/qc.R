basic_qc <- function(input_sce){
  ...
  p1=VlnPlot(input_sce, group.by = "orig.ident", raster=FALSE,features = feats, pt.size = 0, ncol = 2) + 
    NoLegend()
  p1 
  ...
  p2=VlnPlot(input_sce, group.by = "orig.ident", raster=FALSE,features = feats, pt.size = 0, ncol = 3, same.y.lims=T) + 
  ...
  p3=FeatureScatter(input_sce, raster=FALSE,"nCount_RNA", "nFeature_RNA", group.by = "orig.ident", pt.size = 0.5)
  ...
  selected_mito <- WhichCells(input_sce.filt, expression = percent_mito < 50)
  ...
  #可视化过滤后的情况
  feats <- c("nFeature_RNA", "nCount_RNA")
  p1_filtered=VlnPlot(input_sce.filt, raster=FALSE,group.by = "orig.ident", features = feats, pt.size = 0, ncol = 2) + 
    NoLegend()
  w=length(unique(input_sce.filt$orig.ident))/3+5;w 
  ggsave(filename="Vlnplot1_filtered.pdf",plot=p1_filtered,width = w,height = 5)
  
  feats <- c("percent_mito", "percent_ribo", "percent_hb")
  p2_filtered=VlnPlot(input_sce.filt, raster=FALSE,group.by = "orig.ident", features = feats, pt.size = 0, ncol = 3) + 
    NoLegend()
