...
p_umap=DimPlot(sce.all.int, reduction = "UMAP_dim30",raster = F,
               label = T,repel = T) 
p_umap 
...
if(sp=='human'){
   lapply(markers, function(x){
     ...
     w=length( unique(unlist(Idents(sce.all.int))))/6+3;w
  ...
  lapply(markers_list, function(x){
    ...
    DotPlot(sce.all.int , features = genes_to_check )  + 
     # coord_flip() + 
      theme(axis.text.x=element_text(angle=45,hjust = 1))
    h=length( unique(unlist(Idents(sce.all.int))))/6+3;h
    w=length( unique(unlist(genes_to_check)) )/5+6;w
    ggsave(paste('check_for_',x,'.pdf'),width  = w,height = h)
  })
